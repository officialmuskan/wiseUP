...
const Player = () => {
  ...
  const markLectureAsCompleted = async (lectureId) => {
    try {
      ...
      const { data } = await axios.post('/api/user/update-course-progress', { courseId, lectureId }, { headers: { Authorization: `Bearer ${token}` }})
  ...
  const getCourseProgress = async () => {
    try {
      const token = await getToken();
      const { data } = await axios.post('/api/user/get-course-progress', { courseId }, { headers: { Authorization: `Bearer ${token}` }})
